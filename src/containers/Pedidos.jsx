...
function Pedidos() {
  ...
  const pedidosRequest = async () => {
    ...
    const PedidosInfo = await Promise.all(
      jsonResponse.map(async (pedido) => {
        const sedeResponse = await fetch(
          `${import.meta.env.VITE_SEDE}${pedido.sede}/`,
          {
            mode: 'cors',
            method: 'get',
            headers: { Authorization: `Bearer ${accessToken}` },
        ...
        const funcionarioResponse = await fetch(
          `${import.meta.env.VITE_FUNCIONARIO}${pedido.funcionario}/`,
          {
            mode: 'cors',
            method: 'get',
            headers: { Authorization: `Bearer ${accessToken}` },
        ...
        const proveedorResponse = await fetch(
          `${import.meta.env.VITE_PROVEEDOR}${pedido.proveedor}/`,
          {
            mode: 'cors',
            method: 'get',
            headers: { Authorization: `Bearer ${accessToken}` },
  ...
  const [pedidoSeleccionadoDetalle, setPedidoSeleccionadoDetalle] =
    useState(null);
  const [pedidoSeleccionadoEditar, setPedidoSeleccionadoEditar] =
    useState(null);
  const [idPedidoEliminar, setIdPedidoEliminar] = useState(null);
  useEffect(() => {
    setTitle('Pedidos');
    pedidosRequest().then((data) => {
      setPedidos(data);
                        ...
                        {realizado ? (
                                ...
                                {(provided) => (
                                  ...
                                  >
                                    <div
                                      ...
                                      onClick={() =>
                                        setPedidoSeleccionadoDetalle(pedido.id)
                                      }
                                    >
                                      ...
                                      <span className="mb-2 text-xs">
                                        ...
                                        <span className="text-dark ms-sm-2 font-weight-bold">
                                          {pedido.fecha_realizado
                                            ? fechaHora(pedido.fecha_realizado)
                                            : null}
                                        </span>
                                      </span>
                                    ...
                                    >
                                      ...
                                      <EliminarPedido
                                        idPedidoEliminar={idPedidoEliminar}
                          ...
                          <li className="list-group-item border-0 d-flex p-4 mb-2 bg-gray-100 border-radius-lg">
                            ...
                            <div className="ms-auto text-end">
                              ...
                              <button
                                type="button"
                                className="btn btn-block bg-gradient-warning mb-3"
                              >
                                Notification
                              </button>
                              <a
                          ...
                          >
                              ...
                              <span className="mb-2 text-xs">
                                ...
                                <span className="text-dark ms-sm-2 font-weight-bold">
                                  {pedido.fecha_llegada
                                    ? fechaHora(pedido.fecha_llegada)
                                    : null}
                                </span>
                              </span>
                              <span className="text-xs">
                            ...
                            >
                              ...
                              <a
                                className="btn btn-link text-dark px-3 mb-0"
                                onClick={() => {
                                  setPedidoSeleccionadoEditar(pedido.id);
                                  setshowModalEdit(true);
                                }}
                              >
                                <i
