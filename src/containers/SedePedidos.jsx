...
function SedePedidos({ idProveedor }) {
  ...
  useEffect(() => {
    ...
    proveedorRequest()
      ...
      .catch((error) => {
        console.log(
          'Ha ocurrido un error obteniendo la información de proveedor: ',
          error
        );
      });
  }, [setTitle]);
  ...
  const pedidosFiltrados = (idSede) => {
    return pedidos.filter(
      (pedido) => pedido.sede === idSede && pedido.proveedor === idProveedor
    );
  };
                    ...
                    <div className="card-body p-3">
                      <ul className="list-group">
                        <li className="list-group-item border-0 d-flex align-items-center px-0 mb-2">
                          {pedidosFiltrados(item.id) &&
                          pedidosFiltrados(item.id).length ? (
                            pedidosFiltrados(item.id).map((pedido) => {
                              return (
                                <>
                                  <div
                            ...
                            <div className="d-flex align-items-start flex-column justify-content-center">
                              <h6 className="mb-0 text-sm">
                                La sede no tiene pedidos con este proveedor
                              </h6>
                            </div>
                          )}
                        </li>
