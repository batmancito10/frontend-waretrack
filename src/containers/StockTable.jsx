...
function StockTable() {
  ...
  const stockRequest = async () => {
    try {
      const response = await fetch(
        import.meta.env.VITE_SEDE_STOCK.replace(':id', idSede),
        {
          mode: 'cors',
          method: 'get',
  ...
  const patchRequest = async (e) => {
    ...
    if (editStockId === null) {
      ...
      try {
        const response = await fetch(
          import.meta.env.VITE_PATCH_STOCK.replace(':sede', idSede).replace(
            ':producto',
            editStockId
          ),
          {
                              ...
                              {producto.stock.map((stockcito) => (
                                ...
                                >
                                  {editStockId === producto.id ? (
                                    <input
                                      className="form-control w-35 mr-1 d-flex align-items-center"
                                      type="text"
                                      name="stock"
                                      key={stockcito.stock}
                                      value={inputValue}
